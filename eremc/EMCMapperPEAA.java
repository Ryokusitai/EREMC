...
public class EMCMapperPEAA
{
	...
	//↓-------------------------------------------------------------------------------------------------------------------
	public static LinkedHashMap<ItemStack, Integer> IMCregistrations = new LinkedHashMap<ItemStack, Integer>();
	//↑-------------------------------------------------------------------------------------------------------------------
	...
	{
		if (!IMCregistrations.containsKey(stack))
		{
			IMCregistrations.put(stack, value);
			return true;
		}
	...
	{
		SimpleStack sStack;
		for (Entry<ItemStack, Integer> entry : IMCregistrations.entrySet())
		{
			sStack = new SimpleStack(entry.getKey());
			if (entry.getValue() <= 0)
			{
				addToBlacklist(sStack);
			}
			else
			{
				addMapping(sStack, entry.getValue());
			}
		}
	}
	//↑-------------------------------------------------------------------------------------------------------------------
